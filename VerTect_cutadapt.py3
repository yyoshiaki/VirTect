...
def main():
    ...
    parser.add_argument('-F', '--adapt1',  required=True,
                        metavar='adapt1', type=str, help='The forward adapter')
    parser.add_argument('-R', '--adapt2',  required=True,
                        metavar='adapt2', type=str, help='The reverse adapter')
    ...
    os.system('''~/.local/bin/cutadapt -a ''' + adapt1 + ''' -A ''' + adapt2 + ''' -m 40 -q 20,20 -o sample1''' +
              '''"_trimmed_1.fq" -p sample2'''+'''"_trimmed_2.fq" '''+fq1+''' ''' + fq2 + '''''')
